    ...
    <div className="mt-3 border-t border-gray-100 dark:border-slate-800 pt-3">
      ...
      >
        ...
        {secrets.length > 0 && !open && (
          <span className="ml-1 rounded-full bg-indigo-100 dark:bg-indigo-900/40 px-1.5 py-0.5 text-indigo-600 dark:text-indigo-400">
            {secrets.length}
          </span>
        )}
      </button>
          ...
          {loading ? (
            ...
            <>
              {secrets.length === 0 && !showForm && (
                <p className="text-xs text-gray-400 dark:text-slate-500">No secrets set.</p>
              )}
              ...
              {secrets.length > 0 && (
                <div className="rounded-lg border border-gray-100 dark:border-slate-800 overflow-hidden">
                  {secrets.map((s) => (
                    <div
                      key={s.key_name}
                      className="flex items-center justify-between px-3 py-2 gap-3 border-b last:border-b-0 border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-900/50"
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        <KeyRound size={12} className="text-gray-400 dark:text-slate-500 shrink-0" />
                        <code className="text-xs font-mono text-gray-700 dark:text-slate-300 truncate">
                          {s.key_name}
                        </code>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-xs text-gray-300 dark:text-slate-600 font-mono tracking-widest">
                          ••••••••
                        </span>
                        <button
                          onClick={() => handleDelete(s.key_name)}
                          disabled={deleting === s.key_name}
                          className="rounded p-1 text-gray-300 dark:text-slate-600 hover:text-red-500 dark:hover:text-red-400 disabled:opacity-40 transition-colors"
                          title="Delete secret"
                        >
                          <Trash2 size={13} />
                        </button>
